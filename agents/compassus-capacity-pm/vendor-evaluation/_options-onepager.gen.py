...
PARTS = [
    ...
    ("20", "Product quality", INK,
     "Whether they explained how the product decides, or only that it exists."),
...
STEPS = [
    ...
    ("Two marks each", "What they have, and how good it is out of a hundred."),
...
.opt{{display:flex;gap:38px;padding:32px 0 0}}
.opt + .opt{{border-top:1px solid var(--rule);margin-top:30px}}
...
.parts li{{display:grid;grid-template-columns:64px 1fr;grid-template-areas:"t l" ". d";
  column-gap:22px;padding:10px 0;border-bottom:1px solid #E6E8E2}}
...
.why{{margin:22px 0 0;padding:18px 26px;border-left:3px solid var(--gold);background:#FBF7E9}}
.why p{{margin:0;font-size:17px;line-height:1.58;color:#453F2E}}
    ...
    <div class="body">
      ...
      <div class="why">
        <p>Home Care Home Base is a quarter of the score and a <b>tick box rather than a
          judgement</b>, so the priority cannot get quietly watered down as we work through
          sixteen vendors.</p>
        <p>Scope is measured against the one-pager we published, so each vendor is compared to
          <b>what we asked for</b> rather than to one another's marketing. Everywhere else, the
          score rewards a vendor who showed us how their product decides over one who simply
          says it does.</p>
        <p>What comes out is a number we can defend line by line, plus three short lists for
          each vendor: what makes them different, what worries us, and what they did not answer.</p>
      </div>
    </div>
  </div>
