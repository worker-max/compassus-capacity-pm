...
> outside view on what comparable programmes actually deliver, a search for cheaper alternatives, a
...
through one spreadsheet, authorisation is the real bottleneck, pending-auth work is invisible, and
payer rules exist in writing but never reach the moment of decision. The proposed *solution shape*
did not survive. We already own a scheduling optimiser that covers most of the requirement on our
...
any optimiser working is data we do not yet have, and because that first phase is the same
...
- The modal outcome is neither success nor failure. **40–60% of these programmes are never tested
...
**Field service gives the honest ceiling on optimisation gains:** 5–15% travel reduction, 4–15%
productivity. UPS ORION — the best-funded route optimisation programme in the world — landed at
...
**Software licensing is ~10% of total cost in every scenario.** Internal programme labour is 42% of
the base case — four times the licence. Two of the four largest spread drivers are operating-model
...
documented path is HL7v2 and CCD over SFTP. It sells a competing optimiser. 44% Medicare market
share and 97.6% retention mean no substitution leverage. Its own licence agreement bills
...
- **Authorisation is the bottleneck**, sitting upstream of the scheduler's queue where the process
  map does not look.
...
- **Schedule volatility predicts quit risk** — 3,716 nurses, payroll-grade data, 9.2 percentage
  ...
  hold. Honest caveat: no study anywhere isolates scheduling optimisation as the *cause* of a
...
- **Route and efficiency optimisation as a cost lever.** With 70% of clinicians paid per visit, we
...
That is precisely how Alabama failed. **Buying a second optimiser does not address it.**

So the first phase is identical whichever way the platform decision eventually goes:

1. **Measure.** The task census from HCHB workflow records with real handle times. Authorisation
   write-offs. Actual LUPA rate and one-visit-short count. Missed and rebooked visit rates.
   Authorisation turnaround by payer. Income realisation and schedule volatility per clinician.
...
5. **Staff the actual constraint** — authorisation — rather than harvesting scheduler headcount
...
Spend change management ahead of technology. Programmes with excellent change management meet their
...
- Authorisation write-offs and LUPA leakage measured. **If both come back small and the pay-model
  analysis holds, the margin programme does not proceed.**
...
- Baseline exists for the initiative's core metric — quantified capacity and utilisation — which by
  our own KPI table is **not available today**, along with 6 of 8 secondary indicators.
- Scheduler override behaviour and data hygiene are demonstrably manageable in one branch.

## 9. What would change this recommendation

Stated in advance, so the decision is falsifiable:

- Authorisation write-offs come back materially larger than expected. This is the most
