...
> model, organized by where each one binds in the logic rather than by topic. Companion to
...
> attorney and proposes S-45 for insurance authorization. Those assignments collide and the 8.13
...
These are independent tests. A visit can be authorized and still be uneconomic. A visit can be
economic and still be unauthorized. A patient can be inside authorization and out of benefit.
...
| Permission required, yes or no | Independent of payment basis. One large plan dropped home health authorization in 36 states in 2025 | Payer rules library |
...
| Delegated utilization vendor | Network, authorization and claim may sit with a vendor | Payer rules library |
...
An authorization is a per-discipline quantity inside a dated window. It is never a scalar, and a
...
| Window start rule | At least six variants: hospital discharge, discharge to residence, start of care, first service date, authorization start day, calendar year |
...
| Reauthorization lead time | A plan gating at 30 units and expecting contact at 25 fires the workflow five visits early |
...
| Reauthorization calendar | Gate dates are computable in advance, so this is forecastable load |
| Payer mix of census and pipeline | Drives administrative load, not only revenue |
| Pay model per clinician | Determines the marginal cost of one more visit, and whether optimization can produce margin at all |
...
| Wage index market | Labor share is about 75 percent, and travel sits on the non-labor side |

## 4. Where each factor binds

| Decision point | Reimbursement factors that should be present |
|---|---|
| Referral acceptance | SOC capacity, payer class, expected authorization turnaround, benefit window start rule |
...
| Recert and discharge | A new certification period is a new authorization question, a new group and a new threshold |
...
| Authorization burn rate and projected exhaust date | Units consumed per week against units remaining |
| Reauthorization trigger date | Allotment boundary minus payer lead time |
| Benefit-window days consumed before start of care | Referral-to-SOC latency expressed as benefit lost |
| At-risk delivery flag | Visit delivered before authorization confirmed, with backdating status |
...
| Posture | Applies to |
|---|---|
| Read, never weighted | Margin consequence and period utilization against payment. Shown at the moment of decision, never in an objective function |
| Assist | Surfacing payer rules at plan of care, computing reauthorization dates, decrementing counters, flagging floor risk with days and visits remaining |
| Control, rule-based only | Deriving the pending-auth allowance from the payer, deriving window dates from the start rule |
| Never automated | Frequency, discipline substitution against clinical judgment, assignment without human acceptance |

The two highest-value automations — decrementing the counter and computing the reauthorization
trigger — need no payer integration at all. Both are agency-side, and no major clinical system does
them today. Everything requiring payer cooperation waits on the electronic prior authorization
...
- Authorization turnaround by payer, submission to response
