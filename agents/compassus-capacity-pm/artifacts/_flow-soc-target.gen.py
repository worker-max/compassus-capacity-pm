...
def block(x, y, w, h, fill, lines, small=False, badge=None, tc="#fff", bc=None):
    ...
    if badge:
        ...
        add(f'<rect x="{x+w-bw-8}" y="{y-13}" width="{bw}" height="22" rx="11" fill="#FFFFFF" '
            f'stroke="{bcol}" stroke-width="1.7"/>')
        add(f'<text x="{x+w-bw/2-8}" y="{y+2.5}" class="bdg" text-anchor="middle" fill="{bcol}">{esc(badge)}</text>')
...
def xref(x, y, n):
    w = 7.6*len(str(n))+16
    add(f'<rect x="{x+8}" y="{y-13}" width="{w}" height="21" rx="5" fill="#FFFFFF" '
        f'stroke="{MUT}" stroke-width="1.3"/>')
    add(f'<text x="{x+8+w/2}" y="{y+2.5}" class="bdg" text-anchor="middle" fill="{MUT}">{esc(str(n))}</text>')
...
def assist(x, y, w, h, person, lines, small=False, badge=None, n=None):
    ...
    if badge:
        bw = 7.6*len(badge)+18
        add(f'<rect x="{x+w-bw-8}" y="{y-13}" width="{bw}" height="22" rx="11" fill="#FFFFFF" '
            f'stroke="{person}" stroke-width="1.7"/>')
        add(f'<text x="{x+w-bw/2-8}" y="{y+2.5}" class="bdg" text-anchor="middle" fill="{person}">{esc(badge)}</text>')
    if n: xref(x, y, n)

def surf(x, y, w, h, person, lines, small=False, badge=None, n=None):
    block(x, y, w, h, person, lines, small=small)
    add(f'<path d="M {x+6} {y} L {x+w-6} {y} A 6 6 0 0 1 {x+w} {y+6} L {x+w} {y+12} '
        f'L {x} {y+12} L {x} {y+6} A 6 6 0 0 1 {x+6} {y} Z" fill="{ENG}"/>')
    if badge:                       # after the stripe, or the stripe paints over it
        bw = 7.6*len(badge)+18
        add(f'<rect x="{x+w-bw-8}" y="{y-13}" width="{bw}" height="22" rx="11" fill="#FFFFFF" '
            f'stroke="{person}" stroke-width="1.7"/>')
        add(f'<text x="{x+w-bw/2-8}" y="{y+2.5}" class="bdg" text-anchor="middle" fill="{person}">{esc(badge)}</text>')
...
def ghost(x, y, w, h, lines, n=None):
    ...
    for i, ln in enumerate(lines):
        add(f'<text x="{x+w/2}" y="{cy+i*15}" class="bt s" style="fill:{MUT}" '
            f'text-anchor="middle" opacity=".85">{esc(ln)}</text>')
        wl = 6.6*len(ln)
        add(f'<line x1="{x+w/2-wl/2}" y1="{cy+i*15-5}" x2="{x+w/2+wl/2}" y2="{cy+i*15-5}" '
            f'stroke="{MUT}" stroke-width="1.2" opacity=".8"/>')
    lbl(x+w/2, y+h+18, "NO LONGER A STEP", "middle", "trg")
    if n: xref(x, y, n)
...
lbl(W/2, 52, "Home Health Capacity & Scheduling — Detailed Flow  ·  TARGET STATE", "middle", "title")
lbl(W/2, 82, "Read beside the current-state sheet — same five columns, same clean path, same positions"
    "   ·   TARGET STATE v1.0 · PROPOSED, not current state", "middle", "deck")
...
assist(s_pcc, sc-SBH/2, SBW, SBH, C["pcc"], ["Scheduler confirms the", "proposed week"], n="S1")
eng(s_read, sc-SBH/2, SBW, SBH, ["Open capacity read — day ·", "week · discipline · territory"], n="S2")
diamond(d_cap, sc, ["Capacity", "available?"])
diamond(d_match, sc, ["Clean", "match?"])
assist(s_asn, sc-SBH/2, SBW, SBH, C["pcc"], ["Assign visit — proposed,", "scheduler confirms — one pass"], n="S3")
diamond(d_conf, sc, ["Confirmed?"])
man(s_del, sc-SBH/2, SBW, SBH, C["clin"], ["Visit delivered — docu-", "mented, OASIS locked"], n="S4")
...
man(cx(0), y1+196, BW2, BH2, C["dcs"], ["Priority flag", "to scheduling"], n="1.6")
...
arrow(cols[1]+colw/2, y+56, cols[1]+colw/2, y+96)
y += 102
eng(cx(1), y, BW2, BH2, ["Committed load computed", "— points, live"], n="2.3")
arrow(cols[1]+colw/2, y+BH2, cols[1]+colw/2, y+BH2+40)
y += BH2 + 46
eng(cx(1), y, BW2, BH2, ["Assessing capacity", "by discipline"], n="2.4")
arrow(cols[1]+colw/2, y+BH2, cols[1]+colw/2, sc-SBH/2-6)
# pending-auth distortion
ghost(cx(1), y+BH2+52, BW2, 54, ["Pending-auth visits invisible —", "not on calendar, not counted"], n="2.5")
path(f"M {cx(1)+BW2/2-90} {y+BH2+52} L {cx(1)+BW2/2-90} {y+BH2+8}", dash=True)
lbl(cols[1]+colw/2, y+BH2+52+54+38, "the read now includes what is still pending —", "middle", "note")
lbl(cols[1]+colw/2, y+BH2+52+54+56, "it is on the calendar and counted as committed", "middle", "note")

# column 2 — below spine
y2 = SY + SH2 + 56
assist(cx(1), y2, BW2, BH2, C["lead"], ["Branch leadership review —", "reopen zip · adjust · defer"],
       badge="CAPACITY TIGHTENS", n="2.6")
...
y2b = y2 + BH2 + 70
surf(cx(1), y2b, BW2, BH2, C["float_"], ["Engage per-diem / float —", "SOCs or coverage visits"], n="2.7")
...
lbl(cols[2]+colw/2, y+56+18, "the engine routes on drive time, not straight line", "middle", "note")
arrow(cols[2]+colw/2, y+56+26, cols[2]+colw/2, y+96)
y += 102
man(cx(2), y, BW2, BH2, C["hchb"], ["Discipline / role", "match (gate)"], n="3.2")
arrow(cols[2]+colw/2, y+BH2, cols[2]+colw/2, y+BH2+32)
y += BH2 + 38
eng(cx(2), y, BW2, BH2, ["Territory applied —", "zip · drive time"], n="3.3")
arrow(cols[2]+colw/2, y+BH2, cols[2]+colw/2, y+BH2+32)
y += BH2 + 38
assist(cx(2), y, BW2, BH2, C["dcs"], ["Specialty competency", "match"], n="3.4")
arrow(cols[2]+colw/2, y+BH2, cols[2]+colw/2, y+BH2+32)
y += BH2 + 38
assist(cx(2), y, BW2, BH2, C["pcc"], ["Continuity — prefer", "same clinician"], n="3.5")
arrow(cols[2]+colw/2, y+BH2, cols[2]+colw/2, y+BH2+32)
y += BH2 + 38
surf(cx(2), y, BW2, BH2, C["pcc"], ["Patient preferences surfaced — caregiver ·", "window · day-of-week · cognitive"],
     small=True, badge="SHOW, DON'T DECIDE", n="3.6")
arrow(cols[2]+colw/2, y+BH2, cols[2]+colw/2, sc-SBH/2-6)
lbl(cols[2]+colw/2, y+BH2+30, "caregiver presence, the caregiver's own schedule and", "middle", "note")
lbl(cols[2]+colw/2, y+BH2+48, "cognitive constraints stay the scheduler's call — all gating", "middle", "note")

# column 3 — below spine
y3 = SY + SH2 + 110
path(f"M {d_match} {sc+56} L {d_match} {y3-6}", dash=True, label="No",
     lx=d_match+14, ly=sc+80, anchor="start")
assist(cx(2), y3, BW2, BH2, C["dcs"], ["Front-load early visits", "(protect LUPA floor)"], n="3.7")
arrow(cols[2]+colw/2, y3+BH2, cols[2]+colw/2, y3+BH2+40)
y3b = y3 + BH2 + 46
assist(cx(2), y3b, BW2, BH2, C["pcc"], ["Resolve — reassign ·", "resequence · escalate"], n="3.8")
arrow(cols[2]+colw/2, y3b+BH2, cols[2]+colw/2, y3b+BH2+40)
y3c = y3b + BH2 + 46
man(cx(2), y3c, BW2, BH2, C["clin"], ["Clinician receives", "schedule"], n="3.9")
block(cx(2), y3c+BH2+52, BW2, 54, C["auth"],
      ["Auth gate — still checked per visit;", "no auth → pending, but visible and counted"], small=True)
...
add(f'<path d="M {cx(3)+6} {y4b} L {cx(3)+BW2-6} {y4b} A 6 6 0 0 1 {cx(3)+BW2} {y4b+6} '
    f'L {cx(3)+BW2} {y4b+12} L {cx(3)} {y4b+12} L {cx(3)} {y4b+6} A 6 6 0 0 1 {cx(3)+6} {y4b} Z" fill="{ENG}"/>')
xref(cx(3), y4b, "4.3")
arrow(cols[3]+colw/2, y4b+BH2, cols[3]+colw/2, y4b+BH2+38)
y4c = y4b + BH2 + 44
assist(cx(3), y4c, BW2, 66, C["pcc"], ["Coverage broadcast — call · text ·", "Teams — to FT or per-diem", "clinicians who are actually open"], small=True, n="4.4")
arrow(cols[3]+colw/2, y4c+66, cols[3]+colw/2, y4c+66+38)
y4d = y4c + 66 + 44
assist(cx(3), y4d, BW2, 66, C["pcc"], ["Scheduler reassigns to another", "clinician — or moves the visit", "to another day"], small=True, n="4.5")
...
# ==================== COLUMN 5 ====================
y = CT + 420
man(cx(4), y, BW2, BH2, C["clin"], ["Visit in progress", "(EVV / clock-in)"], n="5.1")
lbl(cols[4]+colw/2, y+BH2+22, "the sync lag is measured and shown, not guessed —", "middle", "note")
lbl(cols[4]+colw/2, y+BH2+40, "the office knows how stale the picture is", "middle", "note")
arrow(cols[4]+colw/2, y+BH2+50, cols[4]+colw/2, sc-SBH/2-6)

# column 5 — below spine
y5 = SY + SH2 + 56
man(cx(4), y5, BW2, BH2, C["clin"], ["Documentation", "pending"], n="5.2")
...
y5b = y5 + BH2 + 66
man(cx(4), y5b, BW2, BH2, C["clin"], ["Missed visit —", "clinician documents"], n="5.3")
arrow(cols[4]+colw/2, y5b+BH2, cols[4]+colw/2, y5b+BH2+36)
y5c = y5b + BH2 + 42
eng(cx(4), y5c, BW2, BH2, ["MD notified inside 48h —", "the engine owns the clock"], n="5.4")
lbl(cols[4]+colw/2, y5c+BH2+20, "Medicare requirement and an HCHB hard stop;", "middle", "note")
lbl(cols[4]+colw/2, y5c+BH2+38, "a breach still escalates to the DCS", "middle", "note")
arrow(cols[4]+colw/2, y5c+BH2+48, cols[4]+colw/2, y5c+BH2+78)
y5d = y5c + BH2 + 84
assist(cx(4), y5d, BW2, BH2, C["pcc"], ["Rebook within window ·", "recover / front-load"], n="5.5")
...
lbl(M, H-34, "Compassus Home Health · Capacity & Scheduling — TARGET STATE, PROPOSED. Green = the "
    "capacity & scheduling engine (dark text); purple = still inside HCHB. Posture from the 25 Aug "
    "workbook's future-state column. Phase 1 is visualisation only (DE-03).", cls="foot")
lbl(W-M, H-34, "Continues: Home Health Intake Reset · Page 2", "end", "foot")
add('</svg>')

import sys
OUT = sys.argv[1] if len(sys.argv) > 1 else "soc-target.svg"
