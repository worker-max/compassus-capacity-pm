# Authorization as a Capacity Forecasting Input

> **What this is.** How authorization enters the capacity forecast, which parts of it are tractable,
...
>
> **The short version.** Most of the authorization forecasting problem is deterministic and ignored,
...
| Ceiling truncation | Changes how much demand exists. Frequency is written to clinical need, authorization allows less | Deterministic once the payer rule is known, invisible at the moment it matters |
| Gate events | Creates administrative demand mid-episode at computable points | Arithmetic, not forecasting |

The forecast today is built on ordered frequency, and authorization then truncates it. The demand
curve is therefore systematically overstated for managed patients, and the error is invisible
because nobody compares ordered visits to authorized visits.

## 2. The tractable part is larger than it feels

**The reauthorization calendar.** A plan gating at 30 units with a five-visit lead time produces a
...
**The truncation gap.** Ordered visits minus authorized visits, per discipline, per patient,
...
None of that requires predicting payer behavior. It requires the payer rule and a counter.
...
assessment within 48 hours of referral, which is faster than many plans' authorization turnaround.
...
Advantage plan removed home health prior authorization across 36 states in April 2025, flipping a
...
**Delegated vendor changes.** When a plan moves authorization to a utilization vendor mid-year, the
...
- Observed behavior is compared against the stored rule. When the allowance issued stops matching
  the allowance on file, or turnaround drifts, that is an alert: the rule changed and nobody told
  us.
- Drift is detectable long before a contract amendment is read. The authorization team already sees
...
|---|---|---|
| Committed | Authorized, scheduled, floor-protected | Capacity genuinely spoken for |
| Probable | Pending authorization, weighted by payer approval rate and turnaround | Demand that will land, with a date distribution |
...
- Authorization queue age by payer, by day. Rising queue age today is delayed starts next week.
...
- Truncation gap trend. A widening gap means either plans of care are drifting from payer reality or
  the payer tightened.
- Burn rate against window, per patient. Feeds the reauthorization trigger and catches pooled
  discipline competition early.
- Backdating exposure. Visits delivered against pending authorization still inside their payable
...
The electronic prior authorization requirement effective 1 January 2027 is a step change in
front-door latency, and it lands inside this initiative's scale phase. Design the authorization
