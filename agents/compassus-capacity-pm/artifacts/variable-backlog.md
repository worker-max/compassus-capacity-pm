...
| **Assignment accept / decline + reason** | Scheduling | The Alabama lesson; DE-09 "the tool recommends, the human accepts". **Refined 17 Aug: the decline *fact* is already captured — the scheduler sees that a visit was declined, and it must then go to a clinician other than the original.** | What is missing is the **reason** for the decline, and the accept side. The fact alone cannot train anything; the reason is the training signal |
