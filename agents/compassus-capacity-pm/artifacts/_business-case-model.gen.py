...
rows = [
    ...
    ("", ""),
    ("Colour legend", "BLUE = an input you may edit.  BLACK = a formula, do not overwrite.  GREEN = a link to another sheet.  YELLOW FILL = unresolved or invented, must be replaced before external use."),
    ("", ""),
...
section(ws, 4, "A.  ORGANISATION AND SCALE")
...
unresolved = [
    ("Network admissions per year", 48000, "count",
     "600/branch x 80 (workbook). UNRESOLVED: cannot carry $549M", "Weak"),
    ("Episodic 30-day periods per year", 80000, "count",
...
econ = [
    ("Contribution per recovered admission", 1200, "$", "8.13 workbook", "Workbook", False),
    ("Revenue protected per avoided LUPA", 1400, "$", "8.13 workbook", "Workbook", False),
    ("Contribution per non-episodic visit", 65, "$", "Derived: ~$150 revenue less per-visit pay and variable", "Modelled", False),
    ...
    ("Clinician departures per branch per year", 5, "count", "8.13 workbook", "Workbook", False),
    ("Premium labour pool per branch per year", 120000, "$", "8.13 workbook", "Workbook", False),
...
drivers = [
    ("Admission lift", 0.02, 0.04, 0.07, PCT, "LeanTaaS 2% acute; field service 4-15%"),
    ("Fill-rate points recovered, non-episodic", 0.01, 0.02, 0.035, PCT, "Industry fill rates 88-90%"),
    ...
    ("Premium labour recovery", 0.15, 0.30, 0.50, PCT, "8.13 workbook scenario drivers"),
...
costs = [
    ...
    ("Software licence (per year)", 400000, 720000, 1500000),
    ("Internal programme labour (per year)", 1200000, 3300000, 7000000),
...
for name, lo, mo, hi in costs:
    ...
    r += 1
ws.cell(row=50, column=5, value="Licence is ~10% of total; internal labour ~42%").font = NOTE

section(ws, 52, "E.  BENEFIT RAMP")
hdr(ws, 53, [1, 2, 3, 4, 5], ["Driver", "Year 1", "Year 2", "Year 3", "Basis"])
ws.cell(row=54, column=1, value="Share of steady-state benefit realised").font = BLACK
...
levers = [
    ...
    ("5  Premium and contract labour recovered",
...
cost_rows = [
    ...
    ("Software licence", 46, False),
    ("Internal programme labour", 47, False),
...
ws.cell(row=8, column=1, value="Benefit realised").font = BOLD
for col in (2, 3, 4):
...
base = [
    ...
    ("SOC slot utilisation", "SOC-capable capacity offered versus filled, per week", "Lever 1", "HCHB scheduling", "No",
     "Needs a definition of an SOC slot before it can be counted", "High"),
    ("Visit fill rate", "Authorised and needed visits that go unstaffed, split episodic and non-episodic", "Lever 2", "HCHB", "Partial",
     "Scheduled versus completed versus cancelled, by payer class", "Medium"),
    ...
    ("Premium, contract and overtime spend", "Baseline for the labour recovery lever", "Lever 5", "Payroll", "Yes",
    ...
    ("Income realisation ratio", "Realised pay against quoted pay at 90 days, per new hire", "Lever 6", "Payroll + recruiting", "No",
    ...
    ("Authorisation write-offs", "Visits delivered outside the payer backdating window", "Not yet a lever", "Billing", "No",
     "The most under-instrumented dollar in the business. Could be immaterial or the largest single lever", "High"),
    ("Authorisation turnaround by payer", "Submission to response, by payer and product", "Not yet a lever", "Auth team", "No",
    ...
    ("Cost per period by case-mix group", "The denominator for the utilisation ceiling", "Utilisation work", "Finance", "No",
...
for n in [
    ...
    "By the organisation's own KPI table the initiative's core metric — quantified capacity and utilisation — is not available today, along with 6 of 8 secondary indicators.",
]:
    r += 1
    ws.cell(row=r, column=1, value="- " + n).font = NOTE
