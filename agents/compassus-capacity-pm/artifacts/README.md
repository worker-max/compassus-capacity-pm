...
| **The scenario set** | **Four families, each drawn current state and target state, plus a wide side-by-side comparison. Every sheet names the value levers it demonstrates in a strip at the foot.** | **The executive orientation set.** Together they cover all nine levers |
| `Scenario-Episode-{Current-State,Target-State,Comparison}.pdf` | One referral, one episode. Friday afternoon to the close of the first payment period, with the four points where money leaves marked in sequence. Levers 1, 3, 4, 5 | Showing that the levers are one problem — a deadline nobody sees until it has passed — rather than eight initiatives |
| `Scenario-Clinician-Week-{...}.pdf` | One clinician's week. A nurse who can admit, paid per visit: the week as built, a day inside it, a cancellation she cannot fill, and what the week actually paid her. Levers 2, 3, 8, 9 | The retention argument, and the one to lead with if the room cares about workforce |
| `Scenario-Branch-Week-{...}.pdf` | One branch's week. The Monday queue, the Wednesday call-out, a referral declined with capacity in the building, and a review in which none of the losses appear. Levers 1, 2, 6, 7 | The administrative cost and premium labor argument, and the case for measurement |
| `Scenario-Weekend-Cascade-{...}.pdf` | The weekend cascade. Three weekend starts committed on Friday and what each did to the following week. Lever 1 in depth | The weekend argument. Sunday deferrals, and how a weekend sets or costs the week |
| `_scenario_kit.py`, `_scenario-*.gen.py` | The shared shell and one generator per family. Each takes `current`, `target` or `both` | Regenerating or editing any scenario sheet |
...
| `Flow-Vendor-CareStitch.pdf` | Vendor overlay V3: CareStitch placed on our episode as demoed 23 Sep 2026, every step badged (in CareStitch today · roadmap · beta · opportunity). Same vendor colour as V1 and V2. **Not current state** | Vendor evaluation; comparing where each vendor automates |
| `flow-vendor-carestitch.html` | Source for the above. Regenerate with `python3 _flow-vendor-carestitch.gen.py <out.svg>` then `build.py` | Editing the overlay |
