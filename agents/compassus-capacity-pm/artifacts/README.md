...
| `Future-State-Flow-SOC.pdf` | **The first future-state sheet.** Start of care redesigned: the capacity answer moves to the front, the payer's rules are surfaced before they bite, readiness runs as an agentic voice call (consent for text is not signed until the SOC visit), assignment is proposed and human-confirmed, and the plan-of-care pass is left largely alone | Showing what the platform takes over, what it proposes, and what stays a human call. **Every box is a proposal** — current practice is `Flow-SOC-Full.pdf` |
| `flow-soc-future-state.html` | Source for the above. Regenerate with `_flow-soc-future-state.gen.py` | Editing the future-state flow |
