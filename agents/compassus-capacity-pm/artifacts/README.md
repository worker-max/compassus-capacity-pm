...
| `HH-Scheduling-Master-Project-Plan-BUILT.xlsx` | **The house master project plan, built out around the ten-step approach.** Three new tabs — `Approach — 10 Steps` (the spine: what each step answered, how it was run, what came out, what it hands forward), `Document Register` (36 artifacts, each with what went into it), `Open Items` (11 QA blockers, defects and never-run gates) — plus `Overall Project` reorganised so its bands are the ten steps rather than the ten categories. Every precedent HH Intake rollout tab untouched | Running the initiative; onboarding someone to the approach; the standing open-items list |
| `mvp-spec-sheet.md` | **The day-one scope, derived from the scored inventory** — 19 of 79 variables in scope, 11 gating, the two undecided policy inputs that block the release, which business case levers v1 can and cannot move, and every exclusion with its re-entry trigger | Scoping the first release; briefing a vendor or build team on what phase 1 is |
...
| `Flow-Target-State.pdf` | **The target state — the first and only sheet in the set that is a proposal.** The capacity envelope as a standing band, consulted at the referral decision; the episode in four phases run against it; discharge returning room to the envelope. Every step marked with its release (MVP / V2 / V3) and its automation posture (READ / ASSIST / CONTROL), plus six design questions deliberately left undrawn | Showing where this is going; phasing conversations; briefing a vendor |
| `flow-target-state.html` | Source. Regenerate with `_flow-target-state.gen.py` | Editing the target-state sheet |
